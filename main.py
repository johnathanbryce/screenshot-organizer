...
<dict>
    ...
    if result.returncode == 0:
        ...
        print(
            "Your screenshot organizer will now run automatically even when you close this window."
        )
...
def uninstall_background_service():
    ...
    plist_file = (
        Path.home() / "Library" / "LaunchAgents" / "com.screenshot-organizer.plist"
    )
...
async def main():
    if len(sys.argv) > 1:
        ...
        if mode == "--config":
            ...
            print(
                f"Screenshot Organizer background service started (PID: {os.getpid()})"
            )
